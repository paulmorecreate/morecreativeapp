...
export function PersonDetailClient({ person, talentLinks, allTalents, talentCategories, talentLevels, allAgents, agentTypes, allStylists, allPeople }: Props) {
  ...
  async function handleLinkExisting(talentId: string) {
    ...
    await createClient().from('talent_people').insert({ talent_id: talentId, person_id: person.id })
  ...
  async function handleCreateAndLink(newTalentId: string) {
    await createClient().from('talent_people').insert({ talent_id: newTalentId, person_id: person.id })
    setLinkTalentOpen(false)
    router.refresh()
  }
