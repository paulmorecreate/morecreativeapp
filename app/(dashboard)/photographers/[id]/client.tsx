...
import { Photographer, PhotographerContact } from '@/lib/supabase/types'
...
export function PhotographerDetailClient({ photographer, contacts }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    based: photographer.based ?? '',
    ig_link: photographer.ig_link ?? '',
    website: photographer.website ?? '',
    notes: photographer.notes ?? '',
  })

  const [contactOpen, setContactOpen] = useState(false)
  const [editContact, setEditContact] = useState<PhotographerContact | null>(null)
  ...
  function openEditContact(c: PhotographerContact) {
  ...
  async function handleContactSubmit(e: React.FormEvent) {
    ...
    if (editContact) {
      await supabase.from('photographer_contacts').update(payload).eq('id', editContact.id)
    } else {
      await supabase.from('photographer_contacts').insert(payload)
    }
  ...
  async function deleteContact(id: string) {
    const supabase = createClient()
    await supabase.from('photographer_contacts').delete().eq('id', id)
    router.refresh()
  }

  async function setPrimaryContact(id: string) {
    const supabase = createClient()
    await supabase.from('photographer_contacts').update({ is_primary: false }).eq('photographer_id', photographer.id)
    await supabase.from('photographer_contacts').update({ is_primary: true }).eq('id', id)
    router.refresh()
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href="/photographers" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-700 mb-4">
          <ArrowLeft className="w-3.5 h-3.5" />
          Photographers
        </Link>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">{photographer.name}</h1>
            <div className="flex items-center gap-3 mt-1">
              {photographer.specialty && <span className="text-sm text-gray-500">{photographer.specialty}</span>}
              {photographer.based && <span className="text-sm text-gray-400">{photographer.based}</span>}
            </div>
          </div>
        ...
        <div className="col-span-1 space-y-5">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <dl className="space-y-3">
              <div>
                ...
                <dd className="text-sm text-gray-900">{photographer.specialty ?? <span className="text-gray-300">—</span>}</dd>
              </div>
              <div>
                <dt className="text-xs text-gray-400 mb-0.5">Based</dt>
                <dd className="text-sm text-gray-900">{photographer.based ?? <span className="text-gray-300">—</span>}</dd>
              </div>
              <div>
                <dt className="text-xs text-gray-400 mb-0.5">Instagram</dt>
                <dd className="text-sm">
                  {photographer.ig_link ? (
                    <a href={photographer.ig_link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-gray-900 hover:text-black">
              ...
              <div>
                <dt className="text-xs text-gray-400 mb-0.5">Website / Portfolio</dt>
                <dd className="text-sm">
                  {photographer.website ? (
                    <a href={photographer.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-gray-900 hover:text-black">
          ...
          {photographer.notes && (
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{photographer.notes}</p>
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleContactSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={contactForm.role} onChange={contactField('role')} placeholder="e.g. Agent, Assistant" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
      ...
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Edit Photographer">
      ...
      <Modal open={deleteOpen} onClose={() => setDeleteOpen(false)} title="Delete Photographer">
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-4 bg-red-50 rounded-lg border border-red-100">
            <AlertTriangle className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
            <p className="text-sm text-red-700">
              This will permanently delete <strong>{photographer.name}</strong> and all associated contacts. This cannot be undone.
            </p>
          </div>
          <div className="flex gap-3">
            ...
            <Button type="button" onClick={handleDelete} disabled={deleting} className="flex-1 bg-red-600 hover:bg-red-700 text-white border-red-600">
              {deleting ? 'Deleting…' : 'Delete Photographer'}
            </Button>
          </div>
        </div>
      </Modal>
